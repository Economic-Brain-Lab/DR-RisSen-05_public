...
def main(info, params, trials, intro):
    ...
    # Create orientations for localiser
    result_list = []
    for _ in range(100):
        numbers = np.arange(-90, 91, 1)
        result_list.extend(numbers)

    result_list.sort()
    localiser_array = np.array(result_list)
    # Randomise localiser orientations
    np.random.shuffle(localiser_array)
    ...
    for idx_trial, trial in trials.iterrows():
        ...
        if info.trackEyes:
                ...
                f"Block: {trial['blockNo']}/{nBlocks-1}"]) # This is for eye tracking computer display - experimenter convenience
            ...
            if trial['trialType'] == 1 and showCue:
                ...
                if terminateTrial:
                    ...
                    if not info.simulate:
                        ...
                        if (
                            trial['blockTrialNo'] == (nTrialsPerBlock - 1) and 
                            info.trackEyes and 
                            ...
                            ilnk.endRecording()  
                        breakText.text = '\n'.join([f' End of the practice session',
                            '',
