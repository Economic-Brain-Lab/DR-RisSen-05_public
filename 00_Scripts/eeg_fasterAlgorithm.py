...
from mne.preprocessing.bads import _find_outliers as find_outliers
...
def faster_bad_channels(raw, picks=None, thres=dict(variance=3, correlation=3, hurst=3, kurtosis=3, line_noise=3), use_metrics=None, epochs=False):
    ...
    metrics = {
        ...
        'line_noise':  lambda x: _freqs_power(x, raw.info['sfreq'],
                                              [50, 60]),
    }
    ...
    for m in use_metrics:
        s = metrics[m](data)
        b = [raw.ch_names[picks[i]] for i in find_outliers(s, thres[m])]
...
def faster_bad_epochs(epochs, picks=None, thres=dict(amplitude=3, deviation=3, variance=3), use_metrics=None):
    ...
    for m in use_metrics:
        s = metrics[m](data)
        b = find_outliers(s, thres[m])
        logger.info('Bad by %s:\n\t%s' % (m, b))
        bads.append(b)

    return np.unique(np.concatenate(bads)).tolist()

def faster_bad_sets(erps, picks=None, thres=dict(amplitude=3, deviation=3, variance=3), use_metrics=None):
    """Implements the second step of the FASTER algorithm.
    ...
    Parameters
    ----------
    erps : Array or erps per participant
    picks : list of int | None
...
def faster_bad_components(ica, raw, thres=dict(eog_correlation=3, kurtosis=3, power_gradient=3, hurst=3, median_gradient=3, line_noise=3), use_metrics=None, epochs=False):
    ...
    if epochs:
        source_data = ica.get_sources(raw).get_data().transpose(1,0,2)
    else:
        source_data = ica.get_sources(raw)._data
    source_data = source_data.reshape(source_data.shape[0], -1)

    metrics = {
        'eog_correlation': lambda x: x.find_bads_eog(raw)[1],
...
def faster_bad_channels_in_epochs(epochs, picks=None, thres=dict(amplitude=3, deviation=3, variance=3, median_gradient=3, line_noise=3), use_metrics=None):
    ...
    for m in use_metrics:
        ...
        for i, s in enumerate(s_epochs):
            b = [epochs.ch_names[picks[j]] for j in find_outliers(s, thres[m])]
...
def run_faster(epochs, thres=3, copy=True):
    ...
    epochs.info['bads'] += faster_bad_channels(epochs, thres=thres)
